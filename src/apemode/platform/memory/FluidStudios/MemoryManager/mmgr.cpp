...
// !!IMPORTANT!!
//
// This software is self-documented with periodic comments. Before you start using this software, perform a search for
// the string
// "-DOC-" to locate pertinent information about how to use this software.
//
// You are also encouraged to read the comment blocks throughout this source file. They will help you understand how
// this memory tracking software works, so you can better utilize it within your applications.
//
// NOTES:
//
...
//    running under WIN32, the assert brings up a dialog box, which allocates RAM. Doing this in the middle of an
//    allocation would be bad.
//
...
//    linker errors. Be sure to check those errors each time you compile, otherwise, you may miss a valid linker error.
//
...
//    way (an example is the use of the class MemStaticTimeTracker.)
//
...
// careful if
//    you're including standard includes from within your own project includes; that will break this very specific
//    dependency order. It should look like this:
//
...
// crash (for very good reasons... like making bugs obvious as early as possible.) Some people may be inclined to remove
// this memory tracking software if it causes crashes that didn't exist previously. In reality, these new crashes are
// the BEST reason for using this software!
//
// Whether this software causes your application to crash, or if it reports errors, you need to be able to TRUST this
// software. To this end, you are given some very simple debugging tools.
//
// The quickest way to locate problems is to enable the STRESS_TEST macro (below.) This should catch 95% of the crashes
// before they occur by validating every allocation each time this memory manager performs an allocation function. If
// that doesn't work, keep reading...
//
// If you enable the TEST_MEMORY_MANAGER #define (below), this memory manager will log an entry in the memory.log file
// each time it enters and exits one of its primary allocation handling routines. Each call that succeeds should place
// an "ENTER" and an "EXIT" into the log. If the program crashes within the memory manager, it will log an "ENTER", but
// not an "EXIT". The log will also report the name of the routine.
//
// Just because this memory manager crashes does not mean that there is a bug here! First, an application could
// inadvertantly damage the heap, causing m_internal_malloc(), m_internal_realloc() or m_internal_free() to crash. Also,
// an application could inadvertantly damage some of the memory used by this memory tracking software, causing it to
// crash in much the same way that a damaged heap would affect the standard allocation routines.
//
// In the event of a crash within this code, the first thing you'll want to do is to locate the actual line of code that
// is crashing. You can do this by adding log() entries throughout the routine that crashes, repeating this process
// until you narrow in on the offending line of code. If the crash happens in a standard C allocation routine (i.e.
// malloc, realloc or free) don't bother contacting me, your application has damaged the heap. You can help find the
// culprit in your code by enabling the STRESS_TEST macro (below.)
//
...
// could be cluttered and hard to read.
// ---------------------------------------------------------------------------------------------------------------------------------
...
// assert simply declares a forced breakpoint.
//
...
#define m_assert(x) \
    if ((x) == false) debugger("mmgr: assert failed")
#else
#define m_assert(x) \
    {}
#endif
...
static char *log(const char *format, ...) {
    ...
    /// some / reason.) You can interrogate the variable 'buffer' to see what was supposed to be logged (but won't be.)
    // m_assert(fp);
...
static sAllocUnit *findAllocUnit(const void *reportedAddress) {
    ...
    // allocated addresses will be on four-, eight- or even sixteen-byte boundaries. If we didn't do this, the hash
...
static void wipeWithPattern(sAllocUnit *allocUnit, unsigned long pattern, const unsigned int originalReportedSize = 0) {
    ...
    // to crash in a differnt place each time, so we specifically DO NOT call srand. If, by chance your program calls
    // srand(), you may wish to disable that when running with a random wipe test. This will make any crashes more
    ...
    // the product. So uncomment the following line for releases.
    //
    // Note that the "alwaysWipeAll" should be turned on for this to have effect, otherwise it won't do much good. But
    // we'll leave it this way (as an option) because this does slow things down.
    //	pattern = 0;
    ...
    if (alwaysWipeAll && allocUnit->reportedSize > originalReportedSize) {
        ...
        int i;
        for (i = 0; i < (length >> 2); i++, lptr++) { *lptr = pattern; }
...
static void dumpAllocations(FILE *fp) {
    ...
    fprintf(fp,
            "Number      Reported       Reported        Actual         Actual     Unused    Method  Dealloc Realloc  "
            "Allocated by\r\n");
    fprintf(fp,
            "------ ------------------ ---------- ------------------ ---------- ---------- -------- ------- ------- "
            "---------------------------------------------------\r\n");
    ...
    for (unsigned int i = 0; i < hashSize; i++) {
        ...
        while (ptr) {
            fprintf(fp,
                    "% 6d 0x%016zX 0x%08zX 0x%016zX 0x%08zX 0x%08X %-8s    %c       %c    %s\r\n",
                    ptr->allocationNumber,
                    reinterpret_cast<size_t>(ptr->reportedAddress),
                    ptr->reportedSize,
                    reinterpret_cast<size_t>(ptr->actualAddress),
                    ptr->actualSize,
                    m_calcUnused(ptr),
...
static void dumpLeakReport() {
    ...
    if (stats.totalAllocUnitCount) {
        ...
        if (reservoirBuffer) {
            for (unsigned int i = 0; i < reservoirBufferSize; i++) { m_internal_free(reservoirBuffer[i]); }
    ...
    if (stats.totalAllocUnitCount) { dumpAllocations(fp); }
...
bool &m_breakOnRealloc(void *reportedAddress) {
    ...
    // Interrogate the stack frame or the variable 'au' to see which allocation this is.
    m_assert(au != NULL);

    // If you hit this assert, you tried to set a breakpoint on reallocation for an address that wasn't allocated in a
    // way that is compatible with reallocation.
...
void m_setOwner(const char *file, const unsigned int line, const char *func) {
    ...
    // with no extra parameters.) In order to do this, we use macros that call this function prior to operators new &
    // delete. This is fine... usually. Here's what actually happens when you use this macro to delete an object:
    //
    ...
    // delete. But what happens when we delete an object whose destructor deletes another object, whose desctuctor
    // deletes another object? Here's a diagram (indentation follows stack depth):
    //
    ...
    // that only the final call to delete (in this chain of events) will have the proper reporting, and the first two in
    // the chain will not have ANY owner-reporting information. The deletes will still work fine, we just won't know who
    // called us.
    //
    // "Then build a stack, my friend!" you might think... but it's a very common thing that people will be working with
    // third- party libraries (including MFC under Windows) which is not compiled with this memory manager's macros. In
    // those cases, m_setOwner() is never called, and rightfully should not have the proper trace-back information. So
    // if one of the destructors in the chain ends up being a call to a delete from a non-mmgr-compiled library, the
    // stack will get confused.
    //
    ...
    // this we report a "Possible destructor chain".
    //
...
void *m_allocator(const char *sourceFile,
    ...
    try {
        ...
        if (!reservoir) {
            ...
            for (unsigned int i = 0; i < 256 - 1; i++) { reservoir[i].next = &reservoir[i + 1]; }

            // Add this address to our reservoirBuffer so we can free it later

            sAllocUnit **temp =
                (sAllocUnit **)m_internal_realloc(reservoirBuffer, (reservoirBufferSize + 1) * sizeof(sAllocUnit *));
            m_assert(temp);
            if (temp) {
        ...
        if (allocationType == m_alloc_calloc) { memset(au->reportedAddress, 0, au->reportedSize); }
...
void *m_reallocator(const char *sourceFile,
    ...
    try {
        ...
        // with realloc. In other words, you have a allocation/reallocation mismatch.
        m_assert(au->allocationType == m_alloc_malloc || au->allocationType == m_alloc_calloc ||
                 au->allocationType == m_alloc_realloc);

        // If you hit this assert, then the "break on realloc" flag for this allocation unit is set (and will continue
        // to be set until you specifically shut it off. Interrogate the 'au' variable to determine information about
        ...
        // to the reallocation. This should not happen.
        m_assert(m_validateAllocUnit(au));
...
void m_deallocator(const char *sourceFile,
    ...
    try {
        ...
        if (alwaysLogAll)
            log("[-] ----- %8s of addr 0x%08zX           by %s",
                allocationTypes[deallocationType],
                reinterpret_cast<size_t>(const_cast<void *>(reportedAddress)),
                ownerString(sourceFile, sourceLine, sourceFunc));

        // We should only ever get here with a null pointer if they try to do so with a call to m_internal_free()
        // (delete[] and delete will both bail before they get here.) So, since ANSI allows m_internal_free(NULL), we'll
        // not bother trying to actually free the allocated memory or track it any further.
        ...
        if (reportedAddress) {
            ...
            // probably already know that from a previous assert you should have seen in validateAllocUnit() :)
            m_assert(m_validateAllocUnit(au));
            ...
            // compatible with the deallocation method requested. In other words, you have a allocation/deallocation
            // mismatch.
            ...
            // 'au' variable to determine information about this allocation unit.
            ...
            // WIN32, because Microsoft's memory debugging & tracking utilities will wipe it right after we do. Oh well.
...
bool m_validateAllocUnit(const sAllocUnit *allocUnit) {
    ...
    for (unsigned int i = 0; i < paddingSize; i++, pre++, post++) {
        ...
        // the owner?) has underrun the allocation unit (modified a few bytes prior to the start). You can interrogate
        // the variable 'allocUnit' to see statistics and information about this damaged allocation unit.
        ...
        // If you hit this assert, then you should know that this allocation unit has been damaged. Something (possibly
        // the owner?) has overrun the allocation unit (modified a few bytes after the end). You can interrogate the
...
bool m_validateAllAllocUnits() {
    ...
    // The best way to track this down is to use the alwaysLogAll flag in conjunction with STRESS_TEST macro to narrow
    // in on the offending code. After running the application with these settings (and hitting this assert again),
    // interrogate the memory.log file to find the previous successful operation. The corruption will have occurred
    // between that point and this assertion.
    ...
    // unit in a prior call to validateAllocUnit(), but this assert is here just to make sure you know about it. :)
    m_assert(errors == 0);
...
sMStats m_getMemoryStatistics() { return stats; }
