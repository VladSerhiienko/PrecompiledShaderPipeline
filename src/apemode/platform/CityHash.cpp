...
static uint64 HashLen0to16(const char *s, size_t len) {
    if (len > 8ull) {
        uint64 a = Fetch64(s);
        uint64 b = Fetch64(s + len - 8ull);
        return HashLen16(a, RotateByAtLeast1(b + len, (int)len)) ^ b;
    }
    if (len >= 4ull) {
        uint64 a = Fetch32(s);
        return HashLen16(len + (a << 3ull), Fetch32(s + len - 4ull));
    }
    if (len > 0ull) {
        uint8 a = s[0];
        uint8 b = s[len >> 1ull];
        uint8 c = s[len - 1ull];
        uint32 y = static_cast<uint32>(a) + (static_cast<uint32>(b) << 8u);
        uint32 z = (uint32)len + (static_cast<uint32>(c) << 2u);
...
uint64_t apemode::CityHash64(std::string_view sv) { return CityHash64(sv.data(), sv.size()); }

uint64_t apemode::CityHash64(const char *s, uint64_t len) {
    if (len <= 32) {
        if (len <= 16) {
...
// Hash 128 input bits down to 64 bits of output.
// This is intended to be a reasonably good hash function.
uint64_t apemode::CityHash128to64(uint64_t a, uint64_t b) {
    if (a && b)
        // Murmur-inspired hashing.
