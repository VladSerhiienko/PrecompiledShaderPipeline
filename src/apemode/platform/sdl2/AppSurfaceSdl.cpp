...
apemode::platform::sdl2::AppSurface::~AppSurface() { Finalize(); }
...
bool apemode::platform::sdl2::AppSurface::Initialize(uint32_t width, uint32_t height, const char* name) {
    ...
    if (nullptr == pSdlWindow && !SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS)) {
        ...
        pSdlWindow =
            SDL_CreateWindow(name, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, sdlWindowProps);
...
void* apemode::platform::sdl2::AppSurface::GetWindowHandle() { return reinterpret_cast<void*>(pSdlWindow); }