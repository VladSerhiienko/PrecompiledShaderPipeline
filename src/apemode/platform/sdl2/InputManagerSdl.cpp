...
void AppInputManager::Update(AppInput& appInputState, float const elapsedSeconds) {
    ...
    appInputState.bSizeChanged = false;
    ...
    if (0 != SDL_PeepEvents(events, eventCount, SDL_PEEKEVENT, SDL_WINDOWEVENT, SDL_WINDOWEVENT)) {
        for (int i = 0; i < eventCount; ++i) {
            ...
            if (windowEvent.window.type == SDL_WINDOWEVENT_SIZE_CHANGED ||
                ...
                appInputState.bSizeChanged = true;
                ...
                appInputState.bFocused = false;
            }
        }
    }

    appInputState.bIsQuitRequested = SDL_PeepEvents(nullptr, 0, SDL_PEEKEVENT, SDL_QUIT, SDL_QUIT) > 0;

    // Preserve previous values
    memcpy(appInputState.Buttons[1], appInputState.Buttons[0], sizeof(appInputState.Buttons[0]));

    // Clear tracking flags.
    appInputState.bIsAnyPressed = false;
    appInputState.bIsTrackingTouchesOrMousePressed = false;

    // Clear dynamic per-frame values.
    memset(appInputState.Buttons[0], 0, sizeof(bool) * (kDigitalInput_Touch0));
    memset(appInputState.Analogs, 0, sizeof(bool) * kAnalogInput_Touch0X);
    ...
    if (auto SdlKeybuffer = SDL_GetKeyboardState(nullptr)) {
        for (uint32_t BtnIdx = 0; BtnIdx < kDigitalInput_NumKeys; ++BtnIdx) {
            ...
            appInputState.Buttons[0][BtnIdx] = IsBtnPressed;
            appInputState.bIsAnyPressed |= IsBtnPressed;
        }
    }

    InputManagerUpdateTouches(appInputState, elapsedSeconds);
    InputManagerUpdateMouse(appInputState, elapsedSeconds);

    // Update time duration for buttons pressed
    for (uint32_t BtnIdx = 0; BtnIdx < kDigitalInput_NumInputs; ++BtnIdx) {
        if (!appInputState.Buttons[1][BtnIdx]) {
            appInputState.HoldDuration[BtnIdx] = 0.0f;
        } else if (appInputState.Buttons[0][BtnIdx]) {
            appInputState.HoldDuration[BtnIdx] += elapsedSeconds;
        }
    }

    for (uint32_t AngIdx = 0; AngIdx < kAnalogInput_NumInputs; ++AngIdx) {
        appInputState.AnalogsTimeCorrected[AngIdx] = appInputState.Analogs[AngIdx] * elapsedSeconds;
    }
...
void InputManagerUpdateMouse(AppInput& appInputState, float const elapsedSeconds) {
    ...
    appInputState.bIsAnyPressed |= bIsAnyMousePressed;
    appInputState.bIsTrackingTouchesOrMousePressed |= bIsAnyMousePressed;

    appInputState.Analogs[kAnalogInput_MouseX] = static_cast<float>(RelativeMouseDeltaX);
    appInputState.Analogs[kAnalogInput_MouseY] = static_cast<float>(RelativeMouseDeltaY);

    // Check each available mouse key if it is pressed.
    if ((PressedMouseButtonBitmask & SDL_BUTTON_LMASK) == SDL_BUTTON_LMASK)
        appInputState.Buttons[0][kDigitalInput_Mouse0] = true;
    if ((PressedMouseButtonBitmask & SDL_BUTTON_RMASK) == SDL_BUTTON_RMASK)
        appInputState.Buttons[0][kDigitalInput_Mouse1] = true;
    if ((PressedMouseButtonBitmask & SDL_BUTTON_MMASK) == SDL_BUTTON_MMASK)
        appInputState.Buttons[0][kDigitalInput_Mouse2] = true;
    if ((PressedMouseButtonBitmask & SDL_BUTTON_X1MASK) == SDL_BUTTON_X1MASK)
        appInputState.Buttons[0][kDigitalInput_Mouse3] = true;
...
void InputManagerUpdateTouches(AppInput& appInputState, float const elapsedSeconds) {
    ...
    if (appInputState.bIsUsingTouch) {
        ...
        int peepedEventCount =
            SDL_PeepEvents(peepedEvents, kDigitalInput_TouchMaxCount, SDL_GETEVENT, SDL_FINGERDOWN, SDL_FINGERMOTION);
        ...
        if (peepedEventCount > 0) {
            ...
            for (int i = 0; i < peepedEventCount; ++i) {
                ...
                appInputState.bIsAnyPressed |= bIsFingerTracked;
                appInputState.bIsTrackingTouchesOrMousePressed |= bIsFingerTracked;
                ...
                appInputState.Buttons[0][eFinger] = bIsFingerTracked;
                appInputState.Analogs[eTouchX] = PeepedEvent.tfinger.x;
                appInputState.Analogs[eTouchY] = PeepedEvent.tfinger.y;

                switch (PeepedEvent.type) {
                    case SDL_FINGERDOWN: {
                        appInputState.TouchIds[appInputState.TouchIdCount] = FinderId;

                        auto TouchItEnd = appInputState.TouchIds + appInputState.TouchIdCount + 1;
                        std::sort(appInputState.TouchIds, TouchItEnd);

                        auto TouchIt = std::unique(appInputState.TouchIds, TouchItEnd);
                        appInputState.TouchIdCount = (uint32_t)std::distance(appInputState.TouchIds, TouchIt);
                    } break;

                    case SDL_FINGERUP: {
                        auto TouchItEnd = appInputState.TouchIds + appInputState.TouchIdCount;
                        auto TouchIt = std::find(appInputState.TouchIds, TouchItEnd, FinderId);
