...
uint64_t GetLastModifiedTime(const char* pszFilePath) {
    ...
    if (_stat64i32(pszFilePath, &statBuffer) != 0) return 0;
#else
    struct stat statBuffer;
    if (stat(pszFilePath, &statBuffer) != 0) return 0;
#endif
...
#if _WIN32
    if (!_fullpath(canonicalAbsolutePathBuffer, pszRelativePath, sizeof(canonicalAbsolutePathBuffer))) { return ""; }
#else
    struct stat statBuffer;
    if (stat(pszRelativePath, &statBuffer) != 0) return "";
...
void apemode::platform::shared::AssetManager::Release(const apemode::platform::IAsset* pAsset) const {
    if (const AssetFile* pAssetFile = static_cast<const AssetFile*>(pAsset)) {
        ...
        if (assetIt != AssetFiles.end()) { std::atomic_fetch_sub(&assetIt->second.UseCount, uint32_t(1)); }
    }
}
...
void apemode::platform::shared::AssetManager::UpdateAssets(const char* pszFolderPath,
    ...
    if (storageDirectory.empty()) { storageDirectory = "./"; }
    ...
    if (storageDirectory.size() > 4) {
        ...
        if (addSubDirectories) { storageDirectory = storageDirectory.substr(0, storageDirectory.size() - 2); }
    }
        ...
        auto addFileFn = [&](const char* pszFilePath, tinydir_file file) {
            ...
            if (bMatches) {
                ...
                const std::string relativePath =
                    fullPath.substr(storageDirectoryFull.size() + (storageDirectoryFull.back() != '/'));
...
apemode::vector<uint8_t> TReadFile(const char* pszFilePath) {
    ...
    if (bTextMode) { assetContentBuffer[size] = 0; }
