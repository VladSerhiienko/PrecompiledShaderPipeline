...
struct AssetFile : IAsset {
    ...
    std::vector<uint8_t> GetContentAsTextBuffer() const override;
    std::vector<uint8_t> GetContentAsBinaryBuffer() const override;
...
class FileReader {
public:
    std::vector<uint8_t> ReadBinFile(const char* pszFilePath); /* Returns the content of the file. */
    std::vector<uint8_t> ReadTxtFile(const char* pszFilePath); /* Returns the content of the file. */
};
